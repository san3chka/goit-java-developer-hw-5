...
public class DatabaseUpdateService {
    ...
    public DatabaseUpdateService(Database database) {
        Connection conn = database.getConnection();

        try {
            insertIntoWorkerSt = conn.prepareStatement(
                    "INSERT INTO worker (name, birthday, level, salary) VALUES (?, ?, ?, ?)"
            );

            insertIntoClientSt = conn.prepareStatement(
                    "INSERT INTO client (name) VALUES (?)"
            );

            insertIntoProjectSt = conn.prepareStatement(
                    "INSERT INTO project(client_id, start_date, finish_date) VALUES (?, ?, ?)"
            );

            insertIntoProjectWorkerSt = conn.prepareStatement(
